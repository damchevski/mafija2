...
<h2>Authentication</h2><p>When <a href="ChatEngine.html#connect">ChatEngine#connect</a> is called, it connects to PubNub functions and authorizes the user to access all public channels, and their own channels.</p>
<pre class="prettyprint source lang-js"><code>const ChatEngine = ChatEngineCore.create({
    publishKey: 'demo',
    subscribeKey: 'demo'
});

ChatEngine.connect(username, {}, 'auth-key');</code></pre><p>, it authorizes PubNub PAM permissions for the supplied authentication key on all read and write channels. See <a href="tutorial-topology.html">PubNub Channel Topology</a>.</p>
<p>When the endpoint responds and ChatEngine successfully connects to PubNub, <a href="ChatEngine.html">ChatEngine</a>
emits the <code>$.ready</code> event.</p>
<p>If the authentication call fails, <a href="ChatEngine.html">ChatEngine</a> emits <code>$.error.auth</code>.</p>
