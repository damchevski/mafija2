...
<h3>$ - System Events</h3><p>System events always begin with a <code>$</code>. For example, <a href="ChatEngine.html#event:$%2522.%2522ready">$.ready</a> and <a href="Chat.html#event:$%2522.%2522online%2522.%2522join">$.online.join</a> are examples of events emitted by ChatEninge. They are system events that are automatically emitted when specific things happen. System events are documented in the reference.</p>
<h3>$plugin - PLugin Events</h3><p>Plugin events always begin with <code>$pluginName</code>. <code>$typingIndicator.startTyping</code> is an example of an event emitted by the <a href="module-chat-engine-typing-indicator.html">module:chat-engine-typing-indicator</a> plugin. The <code>$typingIndicator</code> string is a plugin namespace and <code>startTyping</code> is the plugin event. Namespacing plugins helps ensure that there is no collision between plugins.</p>
<h3>Event Children</h3><p>Dots (<code>.</code>) in an event name indicate that the event is a child of some parent. For example <code>image.like</code> indicates that a &quot;like&quot; event was fired for a specific &quot;image&quot;. You could then subscribe to all <code>image</code> events by subscribing to the <code>image.*</code> event. See <a href="tutorial-wildcard.html">Wildcards</a>.</p>
</article>
