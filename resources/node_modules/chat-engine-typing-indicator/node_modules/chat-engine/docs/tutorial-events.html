...
<article>
    <p>Events are the life-blood of ChatEngine. Events let us know when other <a href="User.html">User</a>s do thing in our <a href="Chat.html">Chat</a>.</p>
<p>ChatEngine emit's some events automatically. All ChatEngine events start with <code>$</code>, see <a href="tutorial-namespaces.html">Namespaces</a> for more on that.</p>
<p>Our custom events can be any string, like <code>message</code> or <code>like</code>. Let's define a custom event so we can send and receive text messages between windows.</p>
<h2>Emitting Events to Users in a Chat</h2><p>First, let's <code>emit()</code> a simple text string as a <code>message</code> event over the <a href="Chat.html">Chat</a>. See <a href="Chat.html#emit">Chat#emit</a> for more.</p>
<pre class="prettyprint source lang-js"><code>chat.emit('message', 'Hey, this is Ian!');</code></pre><p>This will broadcast the <code>message</code> event over the internet to all other clients. Subscribe to the event using <a href="Chat.html#on">Chat#on</a> to get notified of the event.</p>
<h2>Subscribing to Events</h2><p>You can subscribe to custom events by supplying any string as first parameter in ```on()````. The second parameter is a callback function that will be run whenever the event is emitted by ANY other user in the same <a href="Chat.html">Chat</a>.</p>
...
<p>Curious about <code>payload.sender</code> and <code>payload.data</code>? See <a href="tutorial-payload.html">Event Payload</a>.</p>
