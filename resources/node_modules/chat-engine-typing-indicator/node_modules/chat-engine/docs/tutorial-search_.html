...
<article>
    <p>Search is a way to retrieve old events that were fired before ChatEngine
...
<p>You can search for events by calling the <a href="Chat.html#search">Chat#search</a> method:</p>
<pre class="prettyprint source lang-js"><code>chat.search({
    event: 'message'
});</code></pre><p><a href="Chat.html#search">Chat#search</a> returns an Event Emitter, so you subscribe to it's events
just like other objects in ChatEngine.</p>
<pre class="prettyprint source lang-js"><code>let searchy = chat.search({
    event: 'message',
    limit: 50
});
...
});</code></pre><p>This will search through every event emitted in the chat until 50 <code>message</code> events
are found or all events have been looked at. The <code>searchy.on</code> event emitter
augments events just like <a href="Chat.html#on">Chat#on</a>.</p>
<p><a href="Chat.html#search">Chat#search</a> is powered by <a href="https://www.pubnub.com/docs/web-javascript/storage-and-history">PubNub History</a> and the same parameters can be input
