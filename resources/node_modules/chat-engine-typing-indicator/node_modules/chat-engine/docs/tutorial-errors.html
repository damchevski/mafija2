...
<article>
    <p>When developing with ChatEngine, errors will occasionally be reported to let you know when things go wrong.</p>
<p>By default, errors are both emitted as <code>$.error</code> events from the object that is responsible and bubbled via <code>throw</code>.</p>
<h2>Thrown Errors</h2><p>You'll notice thrown errors because they usually break Javascript execution and log a stack trace within the console.</p>
<h2>Errors as Events</h2><p>You can also subscribe to errors via the <code>$.error</code> event. If you'd like to subscribe to all ChatEngine errors, try the following:</p>
<pre class="prettyprint source lang-js"><code>ChatEngine.onAny('$.error.*', (payload) {
   console.log(payload);
});</code></pre><h2>Errors in Production</h2><p>In a production app, it is not a good idea to throw errors. If you'd like to suprress errors, supply <code>throwErrors: false</code> in your <code>ceConfig</code>.</p>
