...
Chat.onAny((event, payload) => {
    console.log('something happened', event, payload);
});</code></pre><h3>Namespaced Wildcards</h3><p>Wildcards also work within namespaces.</p>
<h4>System Events</h4><p>You could subscribe to all system events with <code>$.*</code>. See <a href="tutorial-namespaces.html">Namespaces</a>.</p>
<h4>Plugins</h4><p>You can get notified of all plugin events by subscribing to <code>$plugin.*</code>.</p>
